import React from 'react';

export const Footer = () => {
    return (
        <div className="footer_container">
            <h5>Challenge by <a href="https://www.frontendmentor.io/">Frontend Mentor</a></h5>
            <h5>AlexFranciscoDev</h5>
            <div className="ft-social">
                <ul className="ft-social-list">
                    <li>
                        <a href="mailto:alexbcn98@gmail.com">
                        <img src="data:image/png;base64,"/>
                        </a>
                        <a href="https://github.com/AlexFranciscoDev">
                            <img src="data:image/png;base64," />
                        </a>
                    </li>
                </ul>
            </div>
        </div>
    )
}